...
def modify_components(def_lines, backfix, status):
    in_components = False
    new_lines = []
        ...
        if in_components:
            ...
            if line.strip().startswith('-'):
                ...
                if len(tokens) >= 3:
                    ...
                    if master_cell.endswith(backfix):
                        # Find the '+' and status part, replace status
                        parts = line.split('+')
                        if len(parts) == 2:
                            left = parts[0]
                            right = parts[1]
                            ...
                            if right_tokens:
                                # Replace the first token (status) with the desired status
...
def main():
    ...
    new_def_lines = modify_components(def_lines, args.backfix, args.status)
